...
import { rolloverDaily } from './health'
...
export async function loadAll() {
  ...
  try {
    ...
    history = h.merged
    // New day, fresh plan — yesterday's completed daily reminders come
    // back pending (rollover is view-level; the server row is untouched,
    // so each device rolls over in its own morning).
    reminders = rolloverDaily(r.merged)
...
export function toggleReminder(id) {
  ...
  if (changed) {
    // Undo clears the server's day marker too (null = explicit clear), so
    // a row can never sit at done:false with a stale doneOn hanging on it.
    mirror('/reminders/' + encodeURIComponent(id), { method: 'PATCH', body: { done: changed.done, doneOn: changed.done ? changed.doneOn : null } })
  }
  return list
}
