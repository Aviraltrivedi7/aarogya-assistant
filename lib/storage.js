...
export function toggleReminder(id, now = new Date()) {
  const key = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}-${String(now.getDate()).padStart(2, '0')}`
  const list = getReminders().map((r) => {
    if (r.id !== id) return r
    // Toggling OFF-ward while done-on-another-day (yesterday's leftover
    // daily) — a fresh tap today must mark TODAY done, not clear yesterday.
    if (r.done && r.doneOn && r.doneOn !== key) return { ...r, done: true, doneOn: key }
    return { ...r, done: !r.done, doneOn: !r.done ? key : undefined }
  })
  write(KEYS.reminders, list)
  return list
}
