...
// ── Daily reminder reset ─────────────────────────────────────────
// A "daily" reminder marked done must come back PENDING the next day —
// the old boolean stayed done forever, so yesterday's medicine vanished
// from today's plan and notifications never re-fired. `done` stays for
// legacy/UI simplicity but the truth is `doneOn` (a local date key):
// done-for-today ⇔ doneOn === todayKey().
export function todayKey(d = new Date()) {
  return `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}-${String(d.getDate()).padStart(2, '0')}`
}

export function isDoneToday(reminder, now = new Date()) {
  if (!reminder) return false
  return reminder.doneOn === todayKey(now)
}

// Map a stored reminder to the effective view: a daily/weekdays/weekends
// reminder whose doneOn is from an earlier day renders pending again.
// `once`/`today`/`tomorrow` items keep their boolean (they are one-shot —
// once done, they stay done on their day and age out of the plan anyway).
export function rolloverDaily(reminders, now = new Date()) {
  const key = todayKey(now)
  return (reminders || []).map((r) => {
    if (!['daily', 'weekdays', 'weekends'].includes(r.when)) return r
    if (r.doneOn && r.doneOn !== key) return { ...r, done: false }
    return r
  })
}
